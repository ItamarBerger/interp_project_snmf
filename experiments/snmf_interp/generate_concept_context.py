...
def main():
    ...
    json_handler = JsonHandler(["K", "level", "layer", "h_row", "top_activations", "sparsity"], str(save_path), auto_write=False)
    ranks = args.ranks
        ...
        for level_idx, nmf_model in enumerate(pretrained_levels):
            ...
            for concept_idx in range(rank):
                top_indices, top_act = get_top_activating_indices_hierarchical(
                    ...
                    level_idx=level_idx,
                    num_samples=args.num_samples_per_factor,
                )
